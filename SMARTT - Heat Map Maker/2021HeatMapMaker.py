...
try:   
    ...
    if heat_range == 'default':
        if parameter_type == 'Amp':    # WT = ~16
            heat_range = [-20,16]    # Color 1 range
            heat_range2 = [16, 80] # Color 2 range
        elif parameter_type == 'K':    # WT = 1
            heat_range = [apparEnergy(100), apparEnergy(1)]    # Color 1 range
            heat_range2 = [apparEnergy(1), apparEnergy(1/100)] # Color 2 range
        elif parameter_type == 'Ymin':    # WT = ~47
            heat_range = [1, 47]    # Color 1 range
            heat_range2 = [47,90]   # Color 2 range
        elif parameter_type == 'Ymax':    # WT = ~64
            heat_range = [10,64]    # Color 1 range
            heat_range2 = [64, 100] # Color 2 range
                    ...
                    if mutSearch.group(1) != nt:
                        ...
                        try:
                            if parameter_type == 'Amp':
                                if df2[mutant][Amplitude] > df2[mutant][Amplitude_SD]:
                                    mutant_Val = df2[mutant][Amplitude]
                            elif parameter_type == 'K':
                                if df2[mutant][K] > df2[mutant][K_SD]:
