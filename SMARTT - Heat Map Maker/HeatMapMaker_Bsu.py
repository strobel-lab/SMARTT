...
try:   
    # Convert rgb value to hexadecimal format
    ...
    # Determine the apparent change in free energy based on the fold change between the mutation value and the WT value
    def apparEnergy(foldchange):
        ddG = R*T*math.log(foldchange)
        return ddG
    
    # Determine the fill-color based on the value determined by Linear_Change_Colorfract()
    ...
    # Determine the amount of fill shading (color) for a given mutation position [for use in determine_rgb_linear()]
    ...
    construct = 'WT'  # 'WT', 'U87G', 'U185G'
    parameter_type = 'Amp' # Options: 'K', 'Ymin', 'Ymax', 'Amp'
    ...
    fits_generated_with = 'R'  # Options: 'PRISM' or 'R'
    same_scale = False   # Set color scale equal to the WT scale (when set to True)
    ...
    if fits_generated_with == 'PRISM':
        ...
        svgFile = os.path.join(svgFolder, "Bsu_annotated.svg")
        outFile = os.path.join(outFolder, "Bsu_colored_" + parameter_type + '_' + value_type + ".svg")
        
    elif fits_generated_with == 'R':
        if construct == 'WT':
            csvFile = os.path.join(csvFolder, "Bsu-WT_MeanFitValues-withHill-R.csv")
            svgFile = os.path.join(svgFolder, "Bsu-WT_annotated v3.svg")
            outFile = os.path.join(outFolder, "Bsu-WT_colored_" + parameter_type + '_' + value_type + ".svg")
        elif construct == 'U87G':
            csvFile = os.path.join(csvFolder, "Bsu-U87G_MeanFitValues-R.csv")
            svgFile = os.path.join(svgFolder, "Bsu-U87G_annotated v3.svg")
            if same_scale == True:
                outFile = os.path.join(outFolder, "Bsu-U87G_colored_" + parameter_type + '_' + value_type + "_same_scale.svg")
            else:
                outFile = os.path.join(outFolder, "Bsu-U87G_colored_" + parameter_type + '_' + value_type + ".svg")
        elif construct == 'U185G':
            csvFile = os.path.join(csvFolder, "Bsu-U185G_MeanFitValues-R.csv")
            svgFile = os.path.join(svgFolder, "Bsu-U185G_annotated v3.svg")
            if same_scale == True:
                outFile = os.path.join(outFolder, "Bsu-U185G_colored_" + parameter_type + '_' + value_type + "_same_scale.svg")
            else:
                outFile = os.path.join(outFolder, "Bsu-U185G_colored_" + parameter_type + '_' + value_type + ".svg")
    ...
    minAmp_cutoff = 5 # Amp value needs to be greater than this for a K to be considered (~2x the Ymin of WT -- or as you judge appropriate)
    ...
    if heat_range == 'default':
        if construct == 'WT' or same_scale == True:
            if parameter_type == 'Amp':    # WT = 32.6
                heat_range = [5,33]    # Color 1 range
                heat_range2 = [33, 61] # Color 2 range
            elif parameter_type == 'K':    # WT = 1
                heat_range = [apparEnergy(293.5), apparEnergy(1)]    # Color 1 range
                heat_range2 = [apparEnergy(1), apparEnergy(1/293.5)] # Color 2 range
            elif parameter_type == 'Ymin':    # WT = 19.7
                heat_range = [-12, 20]    # Color 1 range
                heat_range2 = [20,52]   # Color 2 range
            elif parameter_type == 'Ymax':    # WT = 52.3
                heat_range = [20,52]    # Color 1 range
                heat_range2 = [52, 84] # Color 2 range
        elif construct == 'U87G':
            if parameter_type == 'Amp':    # U87G = 25.0
                heat_range = [5,25]    # Color 1 range
                heat_range2 = [25, 45] # Color 2 range
            elif parameter_type == 'K':    # U87G = 1
                heat_range = [apparEnergy(293.5), apparEnergy(1)]    # Color 1 range
                heat_range2 = [apparEnergy(1), apparEnergy(1/293.5)] # Color 2 range
            elif parameter_type == 'Ymin':    # U87G = 35.8
                heat_range = [11, 36]    # Color 1 range
                heat_range2 = [36,61]   # Color 2 range
            elif parameter_type == 'Ymax':    # U87G = 60.7
                heat_range = [36,61]    # Color 1 range
                heat_range2 = [61, 86] # Color 2 range
        elif construct == 'U185G':
            if parameter_type == 'Amp':    # U185G = 21.2
                heat_range = [5,21]    # Color 1 range
                heat_range2 = [21, 36] # Color 2 range
            elif parameter_type == 'K':    # WT = 1
                heat_range = [apparEnergy(293.5), apparEnergy(1)]    # Color 1 range
                heat_range2 = [apparEnergy(1), apparEnergy(1/293.5)] # Color 2 range
            elif parameter_type == 'Ymin':    # U185G = 36.5
                heat_range = [16, 37]    # Color 1 range
                heat_range2 = [37,58]   # Color 2 range
            elif parameter_type == 'Ymax':    # U185G = 57.7
                heat_range = [37,58]    # Color 1 range
                heat_range2 = [58, 79] # Color 2 range
    ...
    svgTextVariantsRegRE = re.compile(r"(^t_)([A|U|T|C|G]{1}-?[0-9]{1,3})(_dup[0-9]?)?")
    ...
    if same_scale == True:
        if construct == 'U87G':
            FoldChanges = np.array(df2.loc[K])/np.array(df2['G81T'][K])
        elif construct == 'U185G':
            FoldChanges = np.array(df2.loc[K])/np.array(df2['G179T'][K])
    ...
    for r in rects:
        if 'color' in r['id']:
            ...
            if color_search.group(2) == '1_':                
                ...
                r['style'] = re.sub(r'(fill:)none', r'\1' + fill_color, r['style'])
    ...
    # Initialize Dict to keep track of sample values that pass all filters
    SampleValsDict = {}
    ...
    for p in paths:
            ...
            try:
                ...
                for nt in NUCS:
                    ...
                    if mutSearch.group(1) != nt:
                        ...
                        try:
                            if (df2[mutant][Amplitude] > df2[mutant][Amplitude_SD]) & (df2[mutant][K] > df2[mutant][K_SD]) & (minAmp_cutoff < df2[mutant][Amplitude] < maxAmp_cutoff):
                ...
                SampleValsDict[mutSearch.group(1) + mutSearch.group(2)] = SampleValList
        ...
        if len(SampleValList) < 3:           
            stroke_color = hex_red    
            p['style'] = re.sub(r'(stroke:)#\w{6}', r'\1' + stroke_color, p['style'])
        else:
        ...
        if SampleValue == '':           
            fill_color = hex_white
            p['style'] = re.sub(r'(fill:)#\w{6}', r'\1' + fill_color, p['style'])
            
            stroke_color = hex_grey50    
            p['style'] = re.sub(r'(stroke:)#\w{6}', r'\1' + stroke_color, p['style'])
            continue
        # Set color for each position
            ...
            if parameter_type == 'Amp' or parameter_type == 'Ymax' or parameter_type == 'Ymin':
                ...
                if heat_range2 == None or SampleValue >= heat_range[1]:
                    Colorfract = Linear_Change_Colorfract(SampleValue, cf_slope, cf_intercept)
                    fill_color = determine_rgb_linear(rgb_low_color, rgb_mid_color, Colorfract)
                    #print(p['id'], SampleValue, Colorfract, fill_color)
                else:
                    Colorfract = Linear_Change_Colorfract(SampleValue, cf_slope2, cf_intercept2)
                    fill_color = determine_rgb_linear(rgb_mid_color, rgb_high_color, Colorfract)     
                    #print(p['id'],SampleValue, Colorfract, fill_color)
    ...
    for t in texts:
        ...
        if 't_' in t['id']:
            t_search = svgTextVariantsRegRE.search(t['id'])
            t_id = t_search.group(2)
            #print(len(SampleValsDict[t_id]), t['style'])
            if len(SampleValsDict[t_id]) == 0:
                fill_color = hex_grey50
                t['style'] = re.sub(r'(fill:)#\w{6}', r'\1' + fill_color, t['style'])
            
        elif t['id'] == 'title':
            title_str = ' '.join([value_type.capitalize(), parameter_type, 'by Position'])
            t.contents[0].string = title_str
        elif t['id'] == 'legend_title':
            if parameter_type != 'K':
                title_str = '% Full-length'
                t.contents[0].string = title_str
