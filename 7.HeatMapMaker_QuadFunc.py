...
try:   
 ...
 # Because each mutant will be colored on the map in a different quadrant, the mean functions
 # are not required
    # Determine arithmetic mean
   # def mean(some_list):
    #    new_list = [i for i in some_list]
     #   average = sum(new_list)/len(new_list)
      #  return average
    
    # Determine harmonic mean
    ...
    parameter_type = 'Func' # Options: 'K', 'Ymin', 'Ymax', 'Amp', 'Func'
 # Because each mutation gets colored separately, there is no need for value type  
 # value_type = 'mean' # Options: 'mean', 'best', 'worst', 'h_mean', 'g_mean' -- heatmap color will be based on the best, worst, or mean value of all mutants at each position
    ...
    # File locations
    csvFolder = "/Users/Kathryn/Desktop/AaxSMARTT/AaxSMARTT23.06.14/Fits/"
    svgFolder = "/Users/Kathryn/Desktop/AaxSMARTT/Figures/"
    outFolder = "/Users/Kathryn/Desktop/AaxSMARTT/AaxSMARTT23.06.14/Quad"
    if fits_generated_with == 'PRISM':
        ...
        csvFile = os.path.join(csvFolder, "230630_A9U_cAG.csv")
    svgFile = os.path.join(svgFolder, "AaxU90C2StructureQuad.svg")
    outFile = os.path.join(outFolder, "230630-Aax_A9U_cAG" + parameter_type + ".svg")
    
    
    # Parameter identifiers (column/row titles) used in csv input file
    # ***NOTE: IF ONE OF THE PARAMETERS WASN'T OUTPUT WHEN GENERATING THE FITS, IT NEEDS 
    ...
    minAmp_cutoff = 10 # Amp value needs to be greater than this for a K to be considered (~2x the Ymin of WT -- or as you judge appropriate)
    ...
    # Change based on the WT values for each parameter
    if heat_range == 'default':
        if parameter_type == 'Amp':    # WT = 60, U90C = 20, pyrC = 5% mid point, pyrU = 8% mid point, G10A 40
            heat_range = [0,25]    # Color 1 range 
            heat_range2 = [25, 100] # Color 2 range
        elif parameter_type == 'K':    # WT = 1
            heat_range = [apparEnergy(1000), apparEnergy(1)]    # Color 1 range (goes form -4.3 to 4.3)
            heat_range2 = [apparEnergy(1), apparEnergy(1/1000)] # Color 2 range
        elif parameter_type == 'Ymin':    # WT = 15, U90C = 50
            heat_range = [0, 50]    # Color 1 range
            heat_range2 = [50,100]   # Color 2 range
        elif parameter_type == 'Ymax':    # WT = 75, U90C = 78
            heat_range = [10,80]    # Color 1 range
            heat_range2 = [80, 100] # Color 2 range
        elif parameter_type == 'Func':    # WT = 0
            heat_range = [8,0]    # Color 1 range
            heat_range2 = [0, -8] # Color 2 range

    if parameter_type == 'Amp' or parameter_type == 'Ymax' or parameter_type == 'K' or parameter_type == 'Func':
...
#    else:
 #       print('value_type has unknown designation')
    ...
    # Determine slope and intercept for Colorfract function
    ...
    # Set regex patterns
    # For the quad mutants, the svgVariants now has a third element. It is identical to the csvVariants command
    svgVariantsRegRE = re.compile(r"(^[A|U|T|C|G]{1})(-?[0-9]{1,3})([A|U|T|C|G]{1})")
    csvVariantsRegRE = re.compile(r"(^[A|U|T|C|G]{1})(-?[0-9]{1,3})([A|U|T|C|G]{1})")
    ...
    if fits_generated_with == 'PRISM':
        ...
        df_temp = df.iloc[6:9]
        df_temp.index = [str(ind) + '_SD' for ind in df_temp.index]
        frames = [df.iloc[2:5], df_temp]
        df2= pd.concat(frames).apply(pd.to_numeric)
        #df2=pd.DataFrame(df.values[[2,3,4,6,7,8],:], columns=df.columns, index=df.index[[2,3,4,6,7,8]]).apply(pd.to_numeric)
    
    elif fits_generated_with == 'R':
        df=pd.read_csv(csvFile, sep=',', encoding = "ISO-8859-1", skipinitialspace=True, header=0, index_col = 0)
        #df2=pd.DataFrame(df.values[:,0::2], columns=df.columns[0::2], index=df.index).apply(pd.to_numeric).T
        df2=pd.DataFrame(df.values[:,:], columns=df.columns[:], index=df.index).apply(pd.to_numeric).T
    ...
    if Ymax == None:
        ...
        Ymax_SD = 'Ymax_SD'
        Ymax_SDs = np.sqrt(np.array(df2.loc[Ymin_SD])**2 + np.array(df2.loc[Amplitude_SD])**2)
        Ymax_SDs = pd.DataFrame(Ymax_SDs).T
        Ymax_SDs.columns = df2.columns.values #
        df2.loc[Ymax_SD,:] = Ymax_SDs.loc[0,:]
    elif Amplitude == None:
        ...
        Amplitude_SD = 'Amp_SD'
        Amp_SDs = np.sqrt(np.array(df2.loc[Ymax_SD])**2 + np.array(df2.loc[Ymin_SD])**2)
        Amp_SDs = pd.DataFrame(Amp_SDs).T
        Amp_SDs.columns = df2.columns.values #
        df2.loc[Amplitude_SD,:] = Amp_SDs.loc[0,:]
    elif Ymin == None:
        # Add row -- Amplitude (Amp)
        Ymin = 'Ymin'
        ...
        Ymin_SD = 'Ymin_SD'
        Ymin_SDs = np.sqrt(np.array(df2.loc[Ymax_SD])**2 + np.array(df2.loc[Amplitude_SD])**2)
        Ymin_SDs = pd.DataFrame(Ymin_SDs).T
        Ymin_SDs.columns = df2.columns.values #
        df2.loc[Ymin_SD,:] = Ymin_SDs.loc[0,:]
 ...
 #   ddG_vfunction = np.vectorize(apparEnergy)
  #  ddGs = pd.DataFrame(ddG_vfunction(FoldChanges)).T
   # ddGs.columns = df2.columns.values #
    ...
    # Create Set of all Variant Bases. For the quads, a third element added to the varSet
    varSet = set()
    for variant in df2.columns.values:
        if variant == 'WT':
            ...
            try:
                csvVar = csvVariantsRegRE.search(variant)
                csvVar2add = csvVar.group(1) + csvVar.group(2) + csvVar.group(3)
    ...
    for t in texts:
        ...
        if 'text' in t['id']:
            continue
        if t['id'] == 'title':
            title_str = ' '.join([parameter_type, 'by Position'])
            ...
            if text_search.group(1) == '1':
                if parameter_type == 'Amp' or parameter_type == 'Ymin' or parameter_type == 'Ymax':
                    ...
                    if text_search.group(2) == '0':
                        extreme1 = '\u2264' # this weird symbol gets read as the 'less than or equal to' sign
                        extreme2 = '\u2265' # this weird symbol gets read as the 'greater than or equal to' sign
                else:
                    units = ''
                    if text_search.group(2) == '0':
                ...
                if parameter_type == 'Amp' or parameter_type == 'Ymin' or parameter_type == 'Ymax':
                    ...
                    units = ''
                    if text_search.group(2) == '10':
                        extreme1 = '\u2265' # this weird symbol gets read as the 'greater than or equal to' sign
                        extreme2 = '\u2264' # this weird symbol gets read as the 'less than or equal to' sign
    ...
    for p in paths:
        ...
        if ('path'in p['id']): # these are not named objects (ie. they are not associated w/ a nt position)
            ...
            try:
                ...
                # For the quads, if the mutation is "U", it must be changed to T as well
                mutant = None
                mutant_Val = None
                ...
                if fits_generated_with == 'R':
                    if mutSearch.group(1) == 'U' or mutSearch.group(1) == 'u':
                        mutant = 'T' + mutSearch.group(2) + mutSearch.group(3)
                    elif mutSearch.group(3) == 'U' or mutSearch.group(3) == 'u':
                        mutant = mutSearch.group(1) + mutSearch.group (2) + 'T'
                    else:
                        mutant = mutSearch.group(1) + mutSearch.group(2) + mutSearch.group(3)
          ...
          #          mutant = mutant_base + nt
                    try:
                        if parameter_type == 'Amp':
                            #if df2[mutant][Amplitude] > 0:
                            if df2[mutant][Amplitude_SD] < df2[mutant][Amplitude]:
                                mutant_Val = df2[mutant][Amplitude]
                                #print(mutant_Val)
                            #else:
                            elif df2[mutant][Amplitude_SD] > df2[mutant][Amplitude] and df2[mutant][Amplitude] < minAmp_cutoff:
                             #   mutant_Val = 0
                                mutant_Val = df2[mutant][Amplitude]
                            ...
                            #print("It's OK!!")
                            #print(mutant,": ", df2[mutant][K_SD])
                            if df2[mutant][K]*1.75 > (df2[mutant][K_SD]):
                                #print(mutant,": ", df2[mutant][K])
                                #print(mutant,": ", df2[mutant][Amplitude])
                                if minAmp_cutoff < df2[mutant][Amplitude] < maxAmp_cutoff:
                                    #print(mutant,": ", df2[mutant]['ddG_K'])
                                    mutant_Val = df2[mutant]['ddG_K']
                                    #print(mutant_Val)
                        elif parameter_type == 'Ymin':
                            if df2[mutant][Ymin] > df2[mutant][Ymin_SD]:
                                mutant_Val = df2[mutant][Ymin]

                        elif parameter_type == 'Ymax':
                            if df2[mutant][Ymax] > df2[mutant][Ymax_SD]:
                                mutant_Val = df2[mutant][Ymax]

                        elif parameter_type == 'Func':
                            if df2[mutant][K]*1.5 > (df2[mutant][K_SD]):
                                #print(mutant,": ", df2[mutant][Amplitude])
                            #print('collecting functional data')
                            #print(df2[mutant][Func])
                                if abs(df2[mutant][Func]) > df2[mutant][Func_SD]:
                                    mutant_Val = df2[mutant][Func]
                                #print(df2[mutant][Func])
                                #print(df2[mutant][Func_SD])
                                elif abs(df2[mutant][Func]) < df2[mutant][Func_SD] and df2[mutant][Func_SD] < 1: #0.078596214 is the WT SD 
                                    mutant_Val = df2[mutant][Func]
                    except:
                        mutant_Val = ''
                    #print(mutant_Val)
     ...
     #               elif maxORmin == 'max':
      #                  SampleValue = max(SampleValList)
       #             elif value_type == 'mean':
        ...
        # If one of the mutants was ignored, color the outside of the circle red (or whatever color you want)
#        if len(SampleValList) < 2:           
 #           stroke_color = hex_red    
  #          p_style = re.sub(r'(stroke:)#\w{6}', r'\1' + stroke_color, p_style)
   #         p['style'] = p_style
#        elif len(SampleValList) < 3:
 #           stroke_color = hex_maroon
  #          p_style = re.sub(r'(stroke:)#\w{6}', r'\1' + stroke_color, p_style)
   #         p['style'] = p_style
#        else:
 #           stroke_color = hex_black    
  #          p_style = re.sub(r'(stroke:)#\w{6}', r'\1' + stroke_color, p_style)
   #         p['style'] = p_style
        
        # Set color if all three parameter values were thrown out
        for x in SampleValList:
            if mutant_Val == '':           
                fill_color = hex_grey    
            ...
            # Set color for each position
                ...
                if parameter_type == 'Amp' or parameter_type == 'Ymax' or parameter_type == 'Ymin':
                    if heat_range2 == None or mutant_Val <= heat_range[1]:
                        Colorfract = Linear_Change_Colorfract(mutant_Val, cf_slope, cf_intercept)
                        fill_color = determine_rgb_linear(rgb_low_color, rgb_mid_color, Colorfract)
                    else:
                        Colorfract = Linear_Change_Colorfract(mutant_Val, cf_slope2, cf_intercept2)
                        fill_color = determine_rgb_linear(rgb_mid_color, rgb_high_color, Colorfract)                   
                elif parameter_type == 'K' or parameter_type == 'Func':
                    if heat_range2 == None or mutant_Val >= heat_range[1]:
                        Colorfract = Linear_Change_Colorfract(mutant_Val, cf_slope, cf_intercept)
                        fill_color = determine_rgb_linear(rgb_low_color, rgb_mid_color, Colorfract)
                    else:
