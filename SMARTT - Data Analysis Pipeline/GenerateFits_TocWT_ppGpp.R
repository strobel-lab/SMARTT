...
# alternative to analyzing the data using PRISM.It outputs energetic conversions
# for the collected variables as well as an r_squared value
#
#
# - Requires R (has been tested with version 3.4.4) and the minpack.lm package
# - and the modelr package
# - Public release 1.0
# - Copyright 2022 Kathryn Barth
...
# Name input file
inputfolder <- '/home/kmb242/scratch60/Toc_SMARTT_220221/fitting/Input/'
inputfileName <- '220223-Toc_WT_ppGpp_analysis.csv'
inFile <- paste(inputfolder, inputfileName, sep='')

# Name output file
outputfolder <- './'
outFileName <- '220303-Toc_WT_ppGpp_Fits.csv'
...
mat <- matrix(data = NA, nrow = I(NROW(input_data)-1), ncol = 15)
header <- c('K', 'K_SD', 'Ymin', 'Ymin_SD', 'Ymax', 'Ymax_SD', 'Amp', 'Amp_SD', 'dG_Amp', 'dG_Amp_SD', 
            'dG_K', 'dG_K_SD', 'dG_switch', 'dG_switch_SD', 'r_squared')
colnames(mat) <- header
...
header2 <- c('K', 'K_SD', 'Ymin', 'Ymin_SD', 'Ymax', 'Ymax_SD', 'Amp', 'Amp_SD', 'dG_Amp', 'dG_Amp_SD', 
            ...
            ,'Function', 'Function_SD', 'r_squared')
