...
# Name input file
inputfolder <- 'demos/'
inputfileName <- 'Cte_PFL-demo.csv'
inFile <- paste(inputfolder, inputfileName, sep='')

# Name output file
outputfolder <- 'output/'
outFileName <- 'Cte_PFL-demo_Fits.csv'
...
mat <- matrix(data = NA, nrow = I(NROW(input_data)-1), ncol = 8)
header <- c('K', 'K_SD', 'Ymin', 'Ymin_SD', 'Ymax', 'Ymax_SD', 'Amp', 'Amp_SD')
colnames(mat) <- header
...
for (row in 2:NROW(input_data)){
  ...
  param_vector <- t(c(K_vector,Ymin_vector,Ymax_vector,Amp_val,Amp_SE))
